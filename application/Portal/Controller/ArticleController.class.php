...
//     	}
    	...
    	$newClick = M('posts')->where($moldZH) -> limit('8') ->order('post_hits desc') ->select();
    	...
    	$tplname=sp_get_apphome_tpl($tplname, "newsArticle");
    	//$this->display(":$tplname");
    	$this->display("/zh/newsArticle");
    }
    ...
    function index_en(){
    	...
    	$newClick = M('posts')->where($moldEN) -> limit('8') ->order('post_hits desc') ->select();
    	foreach ($newList as $key => $val){
    		$p = json_decode($val['smeta'],true);
    		$a[$key] = "data/upload/" . $p['thumb'];
    	}
    	...
    	$tplname=sp_get_apphome_tpl($tplname, "newsArticle");
    	//$this->display(":$tplname");
    	$this->display("/en/newsArticle_en");
    }
}
