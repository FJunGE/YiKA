...
class HomebaseController extends AppframeController {
	...
	public function nav(){
        //产品分类导航部分
        $product_nav = M('product_nav') -> limit('9') -> order('id desc') -> select();
        $this -> assign("navl",getCache('product_nav',$product_nav));

        //应用市场导航部分
        $plan_nav = M('plan') -> where('state = 1') ->where('edition=1')-> limit('51') -> order('id desc') ->  select();
        $this->assign("plannav",getCache('plan_nav',$plan_nav));

        //网站配置
        $setting = M('options')->where("option_name='site_options'")->field('option_value')->select();
        $this->assign('setting',json_decode($setting[0]['option_value'], true));
    }
